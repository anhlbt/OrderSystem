<template>
  <div id="choose">
    <van-nav-bar title="Start ordering" fixed left-arrow @click-left="$router.push('/mealOrder')" />
    <section>
    ...
    <footer>
      ...
      <span>{{total}}</span>
      <button @click="orderPopup=true" class="van-button--primary">submit</button>
    </footer>

    <van-popup v-model="cartPopup" position="bottom" :style="{ height: '40%'}">
      <div class="popup">
        <div class="popup-hd"><span>Selected dishes</span></div>
        <div class="popup-bd">
          <div class="goods">
        ...
        <div class="popup-ft">
          <span>{{total}}</span>
          <button @click="orderPopup=true">submit</button>
        </div>
      </div>
    </van-popup>

    <van-popup v-model="orderPopup" position="bottom" :style="{height: '100%'}">
      <van-nav-bar title="Order Confirmation" left-arrow @click-left="orderPopup = false" />
      <section>
        <van-divider>Dishes</van-divider>
        <div class="goods">
          <div class="item" v-for="(e,i) in cart" :key="i">
            <div class="hd">
              <img :src="e.url" alt="">
            ...
            <div class="ft">
              <span>{{e.num}}/share</span>
            </div>
          </div>
        </div>
        <div class="total">total:<span>{{total}}</span>Yuan</div>
        <van-divider>Remark</van-divider>
        <van-field v-model="remark" rows="3" autosize type="textarea" maxlength="50" placeholder="Please fill in the information such as what the guests have to avoid" show-word-limit />
      </section>
      <footer>
        <button @click="submitOrder" class="van-button--primary">submit</button>
      </footer>
    </van-popup>
  ...
  methods: {
    ...
    async submitOrder() {
      if (this.cart.length == 0) {
        this.$toast("Please select any dishes and cannot submit");
        return;
      }
      ...
      if (this.isEdit) {
        ...
        try {
          ...
          this.$toast("Submission Failed" + error);
          return;
        }
        try {
          //提交订单
